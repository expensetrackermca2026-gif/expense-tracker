...
def add_expense():
    ...
    new_exp = Expense(
        ...
        amount=abs(amount),
        category=category,
        type="Paid" if amount > 0 else "Received",
...
def parser():
    ...
    if request.method == "POST":
        ...
        if file and file.filename.endswith(".pdf"):
            ...
            now = datetime.utcnow()
            user_statement_dir = os.path.join(
            ...
            try:
                ...
                if (
                    StatementArchive.query.filter_by(
                        user_id=u_id, original_file_name=filename
                    ).first()
                    or ActiveParserTransaction.query.filter_by(
                        user_id=u_id, upload_batch=filename
                    ).first()
                ):
                ...
                existing_parsed = ActiveParserTransaction.query.filter_by(
                    user_id=u_id
                ).count()
                archived_count = 0
                if existing_parsed > 0:
                    _, archived_count = _archive_parsed_transactions(
                        u_id, filename, db.session
                    )
    ...
    expenses = (
        ActiveParserTransaction.query.filter_by(user_id=u_id)
        .order_by(ActiveParserTransaction.date.desc())
        .all()
    )
    p_paid = (
        db.session.query(func.sum(ActiveParserTransaction.amount))
        .filter_by(user_id=u_id, type="Paid")
        .scalar()
        or 0
    )
    p_received = (
        db.session.query(func.sum(ActiveParserTransaction.amount))
        .filter_by(user_id=u_id, type="Received")
        .scalar()
        or 0
    )

    return render_template(
        "parser.html", expenses=expenses, p_paid=p_paid, p_received=p_received
    )
...
def statement_history():
    ...
    u_id = session["user_id"]
    archives = (
        StatementArchive.query.filter_by(user_id=u_id)
        .order_by(StatementArchive.archived_at.desc())
        .all()
    )
    ...
    for arc in archives:
        txns = (
            ArchiveTransaction.query.filter_by(archive_id=arc.archive_id)
            .order_by(ArchiveTransaction.date.desc())
            .all()
        )
        result.append(
            {
...
def receipts():
    ...
    images = Expense.query.filter(
        Expense.user_id == u_id, Expense.attachment_url != None
    ).all()
...
def analyze_receipt():
    ...
    api_key = os.getenv("GOOGLE_API_KEY")
    if not api_key:
        return {"error": "AI Config Missing"}, 500
    ...
    try:
        ...
        try:
            ...
            return {
                "success": False,
                "error": "AI blocked the prompt due to safety concerns.",
            }, 400
        except Exception as e:
            with open("ai_error_log.txt", "a") as log:
...
def manual():
    ...
    u_id = session["user_id"]
    expenses = (
        Expense.query.filter_by(user_id=u_id, is_parsed=False, attachment_url=None)
        .order_by(Expense.expense_date.desc())
        .all()
    )
    m_paid = (
        db.session.query(func.sum(Expense.amount))
        .filter_by(user_id=u_id, is_parsed=False, type="Paid")
        .scalar()
        or 0
    )
    m_received = (
        db.session.query(func.sum(Expense.amount))
        .filter_by(user_id=u_id, is_parsed=False, type="Received")
        .scalar()
        or 0
    )
    cat_sum = (
    ...
    for i in range(6, -1, -1):
        ...
        amt = (
            db.session.query(func.sum(Expense.amount))
            .filter_by(user_id=u_id, is_parsed=False, type="Paid")
            .filter(func.date(Expense.expense_date) == d)
            .scalar()
            or 0
        )
        daily_labels.append(d.strftime("%b %d"))
        daily_values.append(amt)
    in_group = False
    ...
    return render_template(
        ...
        pie_values=[r[1] for r in cat_sum],
...
def export_csv():
    ...
    u_id = session["user_id"]
    expenses = (
        Expense.query.filter_by(user_id=u_id)
        .order_by(Expense.expense_date.desc())
        .all()
    )
    ...
    for e in expenses:
        data.append(
            {
                "Date": e.expense_date.strftime("%Y-%m-%d %H:%M:%S")
                if e.expense_date
                else "",
                "Title": e.title,
    ...
    return send_file(
        ...
        mimetype="text/csv",
        as_attachment=True,
        download_name="ExpenseAI_Report.csv",
    )
...
def export_pdf():
    ...
    for e in expenses:
        ...
        c.drawString(
            50, y, e.expense_date.strftime("%Y-%m-%d") if e.expense_date else ""
        )
        ...
        if e.type == "Paid":
            total_paid += e.amount
        elif e.type == "Received":
            total_received += e.amount
    ...
    return send_file(
        ...
        as_attachment=True,
        download_name="ExpenseAI_Report.pdf",
    )
